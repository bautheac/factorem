...
setMethod("plot_performance", "AssetPricingFactor", function(factor) {
  ...
  dygraph::dygraph(data %>% dplyr::filter(leg == paste(factor@name, "factor", sep = " ")) %>% dplyr::select(date, `wealth index`),
                   main = paste(factor@name, "factor", sep = " "),
                   group = "factor")
  dygraph::dygraph(data %>% dplyr::filter(leg == "long leg") %>% dplyr::select(date, `wealth index`),
                   main = "long leg",
                   group = "factor")
  dygraph::dygraph(data %>% dplyr::filter(leg == "short leg") %>% dplyr::select(date, `wealth index`),
                   main = "short leg",
                   group = "factor")
